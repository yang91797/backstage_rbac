...
from rbac.service.re_urls import memory_reverse
...
def menu_list(request):
    ...
    menu_exists = models.Menu.objects.filter(id=menu_id).exists()   # 查询一级菜单是否存在
    if not menu_exists:
        menu_id = None      # 不存在，则二级菜单的新建按钮不会显示

    if menu_id:
        second_menus = models.Permission.objects.filter(menu_id=menu_id)    # 二级菜单
    else:
        second_menus = []

    second_menu_exists = models.Permission.objects.filter(id=menu_id).exists()
...
def multi_permissions(request):
    ...
    update_formset = None
    if request.method == 'POST' and post_type == 'generate':    # 批量增加
    ...
    if not generate_formset:
        ...
        # formset_add_class(initial=[{'name': 'rbac:role_list', 'url': '/rbac/role/list/'}, {'name': 'rbac:role_add', 'url': '/rbac/role/add/'}])
        generate_formset = generate_formset_class(initial=[
            row_dict for name, row_dict in all_url_dict.items() if name in generate_name_list
        ])
...
def distribute_permissions(request):
    ...
    for row in all_permission_list:
        ...
        if not pid:
            continue
        all_second_dict[pid]['children'].append(row)
    return render(
        request,
