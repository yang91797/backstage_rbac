...
def user_list(request):
    ...
    return render(request, 'rbac/user_list.html', {'user': user_queryset})
...
def user_add(request):
    ...
    if form.is_valid():
        form.save()
        return redirect(reverse("rbac:user_list"))      # 反向生成url
    return render(request, 'rbac/change.html', {'form': form})
...
def user_edit(request, pk):
    ...
    if request.method == "GET":
        form = UpdateUserModelForm(instance=obj)            # instance 输入框默认值
        return render(request, 'rbac/change.html', {'form': form})
